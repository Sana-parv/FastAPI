...
async def render_todo_page(request: Request,db: db_dependency):
    try:
        ...
        return templates.TemplateResponse(name="todo.html",
                                          request=request,
                                          context={
                                              "request": request,
                                              "todos": todos,
...
#### endpoints ####
#It returns only the todos that belong to the currently logged-in user
@router.get("/")
...
#This endpoint is used to fetch one specific todo belonging to the currently logged-in user.
...
async def delete_todo(user:user_dependency, db: db_dependency , todo_id: int = Path(gt=0)):
    ...
    db.query(Todo).filter(Todo.id == todo_id).filter( Todo.owner_id == user.get('user_id')).delete()
